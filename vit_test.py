...
#    checkpoint_filepath ="model_bak.hdf5"
    checkpoint_filepath ="1000_4/model_10.hdf5"
...
#    with open('epoch_patchsize.txt','a') as f:
#        f.write(str(num_epochs)+"    "+str(patch_size)+"    "+str(accuracy)+"\n")
    print(f"############################\nTest accuracy: {round(accuracy * 100, 2)}%\n##########################")
#    print(f"Test top 5 accuracy: {round(top_5_accuracy * 100, 2)}%")
##    layers = model.layers
#    last_layer_name = layers[-50].name
#    print("Last layer name:", last_layer_name)
##    last_layer_name = 'multi_head_attention_7'
##    attention_map(model,last_layer_name)

def get_attention_weights(model, layer_name):
    layer = model.get_layer(name=layer_name)
    if 'MultiHeadAttention' in layer.__class__.__name__:
        return layer.attention_weights
    else:
        raise ValueError(f"Layer {layer_name} is not a MultiHeadAttention layer.")
def attention_map(model, attention_layer_name):
#    attention_layer_name = 'dense_19'
    attention_weights = get_attention_weights(model, attention_layer_name)
    with tf.GradientTape() as tape:
        outputs = model(img_array)
        tape.watch(attention_weights)
        attention_map = tf.reduce_mean(attention_weights, axis=1).numpy().squeeze()
